...
DataAccess.prototype.getPagesBySiteId = function( id, callback ) {
	this.connect(function( err, client ){
		if ( err ) {
			callback( err );
			return;
		...
		client
		   .query({
			   text: 'SELECT p.* FROM page p WHERE p.site_id=$1 AND p.active = true;',
			   name: 'get-pages-by-siteid',
			   values: [
				   id
			   ]
		   })
			  ...
			  .on( 'error', function( err ){
				  callback( 'Error with query: get pages by site id: ' + err );
			  });
	});
};
